...
// SortedIndentedStrings given a []string that has items with different levels of indent that are used to
// indicate parent -> child relationships, sorts the items case-insensitively with child items sorted
// underneath their parent items, and so on recursively to any level of depth.
// The amount of indent per level is computed by finding the first indented item. Indentation must either
// be one or more spaces or one or more tabs.
